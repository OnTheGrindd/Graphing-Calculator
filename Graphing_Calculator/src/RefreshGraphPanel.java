...
public class RefreshGraphPanel extends JPanel implements MouseListener {
	...
	public void paint(Graphics g) { // overrides paint() in JPanel 
		...
		int[][] yTickPix = getyPixelVals(yPixelInterval, yComponents);
                ...
                for(int i = 0;i<yTickPix[0].length;i++){
                    g.drawLine(windowWidth/2-5,yTickPix[1][i],windowWidth/2+5,yTickPix[1][i]);
                }
		for(int i = 0;i<xTickPix[0].length;i++){
                    g.drawLine(xTickPix[1][i],windowHeight/2-5,xTickPix[1][i],windowHeight/2+5);
                }
                //System.out.println(xTickPix[0][0]);
                for(int i = 0;i<xTickPix[0].length;i++){
                System.out.println(xTickPix[1][i]);}
	...
	public int[][] getxPixelVals(double interval, String[] values) {
		...
		for(int i = 0; i < values.length*interval; i+= interval){
			if(Double.parseDouble(values[index]) >= 0) pixels[1][index] = (int)(i); 
			else pixels[1][index] = (int)(i);
